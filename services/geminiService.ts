...
export const generatePresentationData = async (text: string): Promise<PresentationData> => {
  ...
  try {
    // Use direct REST API call as in the original version
...
export const generatePptx = async (data: PresentationData): Promise<Blob> => {
  ...
  s1.addText("Casal Notarial", {
    x: 0, y: 0, w: 5, h: 5.625, fontSize: 44, color: WHITE, bold: true, align: "center", valign: "middle",
  });
  s1.addShape(pres.ShapeType.rect, { x: 5, y: 0, w: 5, h: 5.625, fill: { color: GRAY_LIGHT } });
  s1.addText(
    [
      { text: `🎥 Convidada: ${data.cover.guestName}\n`, options: { fontSize: 11, bold: true } },
      { text: `⚖️ Área: ${data.cover.area}\n`, options: { fontSize: 11 } },
      { text: `🎻 Tema: ${data.cover.theme}\n`, options: { fontSize: 11 } },
      { text: `📱 Instagram: ${data.cover.instagram}\n`, options: { fontSize: 11 } },
      ...(data.cover.linkedin ? [{ text: `💼 LinkedIn: ${data.cover.linkedin}\n`, options: { fontSize: 11 } }] : []),
    ],
    { x: 5.2, y: 0.4, w: 4.6, h: 1.6, color: GRAY_TEXT, valign: "top" }
  );

  s1.addText(data.cover.title, { x: 5.2, y: 2.1, w: 4.6, h: 0.9, fontSize: 18, color: BLUE, bold: true, valign: "top" });
  s1.addText(`DURAÇÃO: ${data.cover.duration}`, { x: 5.2, y: 3.1, w: 4.6, h: 0.3, fontSize: 10, color: GRAY_TEXT, bold: true, valign: "top" });
  s1.addText(data.cover.guestDescription, { x: 5.2, y: 3.5, w: 4.6, h: 1.4, fontSize: 9, color: GRAY_TEXT, valign: "top" });
  s1.addText(`OLHAR CENTRAL: ${data.cover.centralGoal}`, { x: 5.2, y: 5.0, w: 4.6, h: 0.5, fontSize: 9, color: ORANGE, italic: true, valign: "bottom" });

  const addHeader = (slide: PptxGenJS.Slide, title: string) => {
    slide.addShape(pres.ShapeType.rect, { x: 0, y: 0, w: 10, h: 0.85, fill: { color: BLUE } });
    slide.addShape(pres.ShapeType.rect, { x: 0.3, y: 0.1, w: 0.65, h: 0.65, fill: { color: WHITE } });
    slide.addText("Notar-IA", { x: 0.3, y: 0.1, w: 0.65, h: 0.65, fontSize: 8, color: BLUE, bold: true, align: "center", valign: "middle" });
    slide.addText(title, { x: 1.2, y: 0.1, w: 8.5, h: 0.65, fontSize: 24, color: WHITE, bold: true, valign: "middle" });
  };

  const s2 = pres.addSlide();
  s2.background = { color: WHITE };
  addHeader(s2, "ABERTURA");
  const openRows = data.opening.points.map((p, i) => [{ text: `${i + 1}. ${p}`, options: { fontSize: 14 } }]);
  if (openRows.length > 0) {
    s2.addTable(openRows, { x: 0.5, y: 1, w: 9, border: { type: "none" }, color: GRAY_TEXT });
  }

  s2.addShape(pres.ShapeType.rect, { x: 0.5, y: 3.2, w: 9, h: 0.8, fill: { color: "F9FAFB" } });
  s2.addShape(pres.ShapeType.rect, { x: 0.5, y: 3.2, w: 0.1, h: 0.8, fill: { color: ORANGE } });
  s2.addText(`GANCHO: ${data.opening.hook}`, { x: 0.7, y: 3.2, w: 8.6, h: 0.8, fontSize: 12, color: GRAY_TEXT, valign: "middle" });
  s2.addShape(pres.ShapeType.rect, { x: 0.5, y: 4.2, w: 9, h: 0.6, fill: { color: BLUE } });
  s2.addText(`CTA: ${data.opening.cta}`, { x: 0.5, y: 4.2, w: 9, h: 0.6, fontSize: 12, color: WHITE, bold: true, align: "center", valign: "middle" });

  data.blocks.forEach((block) => {
    const slide = pres.addSlide();
    slide.background = { color: WHITE };
    addHeader(slide, block.title.toUpperCase());
    slide.addText(`Objetivo: ${block.objective}`, { x: 0.5, y: 1, w: 9, fontSize: 12, color: GRAY_TEXT, italic: true });
    slide.addText("Perguntas", { x: 0.5, y: 1.4, fontSize: 16, color: BLUE, bold: true });
    slide.addShape(pres.ShapeType.line, { x: 0.5, y: 1.7, w: 9, h: 0, line: { color: ORANGE, width: 2 } });
    ...
    if (block.questions.length > 5) {
      ...
      const createRows = (qs: string[], offset: number) => qs.map((q, i) => [
        { text: `${offset + i + 1}.`, options: { fontSize: 11, color: ORANGE, bold: true } },
        { text: q, options: { fontSize: 11, color: GRAY_TEXT } },
      ]);
      slide.addTable(createRows(col1, 0), { x: 0.5, y: 1.9, w: 4.2, border: { type: "none" }, colW: [0.3, 3.9] });
      slide.addTable(createRows(col2, half), { x: 5.0, y: 1.9, w: 4.2, border: { type: "none" }, colW: [0.3, 3.9] });
    } else {
      ...
      if (qRows.length > 0) {
        slide.addTable(qRows, { x: 0.5, y: 1.9, w: 9, border: { type: "none" }, colW: [0.3, 8.7] });
      }
    }
  });
  ...
  s11.addText("O momento mais esperado:", { x: 0, y: 1.5, w: 10, fontSize: 16, color: BLUE, align: "center" });
  s11.addShape(pres.ShapeType.rect, { x: 1, y: 2.2, w: 8, h: 1.5, fill: { color: GRAY_LIGHT } });
  s11.addShape(pres.ShapeType.rect, { x: 1, y: 2.2, w: 0.15, h: 1.5, fill: { color: ORANGE } });
  s11.addText(data.finalQuestion.question, { x: 1.3, y: 2.2, w: 7.5, h: 1.5, fontSize: 16, color: GRAY_TEXT, bold: true, align: "center", valign: "middle" });
  s11.addText("⏱️ Duração: 5 minutos", { x: 0, y: 4.2, w: 10, fontSize: 14, color: GRAY_TEXT, align: "center" });
  ...
  s12.addShape(pres.ShapeType.line, { x: 0.5, y: 1.4, w: 4, h: 0, line: { color: ORANGE, width: 2 } });
  const recapRows = data.closing.recapPoints.map((p) => [{ text: `🎙️ ${p}`, options: { fontSize: 12 } }]);
  ...
  s12.addShape(pres.ShapeType.line, { x: 0.5, y: 3.9, w: 4, h: 0, line: { color: ORANGE, width: 2 } });
  s12.addShape(pres.ShapeType.rect, { x: 0.5, y: 4.1, w: 9, h: 1.2, fill: { color: GRAY_LIGHT } });
  const ctaRows = data.closing.ctaPoints.map((p) => [{ text: `📱 ${p}`, options: { fontSize: 12 } }]);
  if (ctaRows.length > 0) {
